bool vyp;
uint16_t cas_com;   //kontrola kamunikacie
...
int8_t Modbus::getRxBuffer() {
  ...
  while ( port->available() ) {
    ...
    u8BufferSize ++;
    cas_com=0;                                            //nulovanie casovaca ak komunikuje
...
*/
  ...
  u16OutCnt++;
//  if (u8txenpin > 1) { digitalWrite( u8txenpin, LOW );}   //RS 485
  if (u8txenpin > 1) {vyp=HIGH;}   //RS 485  oneskorene vypnutie
